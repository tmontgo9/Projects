...
float Integral(float functionId, float a, float b, int n, int intensity, int nbthreads, int granularity){
  SeqLoop sl(n, nbthreads, granularity);
  float eq1 = (float)(b-a)/n;
  float eqans = 0;
  std::vector<float> eqansv;
...
int main (int argc, char* argv[]) {
  ...
  float response = (float) Integral(functionId, a, b, n, intensity, nbthreads, granularity);
